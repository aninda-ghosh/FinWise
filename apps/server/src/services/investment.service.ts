...
export async function refreshPrice(
  id: string
): Promise<{ source_url: string; price: number; price_inr: number; currency: string; fetched_at: string }> {
  const db = getDb();
  const rates = await getLatestRates();
  ...
  const purchaseInr = currency === "INR" ? inv.purchase_value : inv.purchase_value * (rates[currency] ?? 1);
  const currentInr = currency === "INR" ? inv.current_value : inv.current_value * (rates[currency] ?? 1);
  ...
  if (!result.price || result.price <= 0) {
    throw Object.assign(
      new Error(`Could not find a valid market price for "${inv.name}" — try updating the value manually`),
      { status: 422 }
    );
  }
...
export async function getPortfolioSummary() {
  ...
  for (const inv of invList) {
    byAssetType[inv.asset_type] = (byAssetType[inv.asset_type] ?? 0) + inv.current_value_inr;
  }

  // Linked accounts (off-budget savings / investment accounts)
  const linkedAccounts = allAccounts.filter(
    a => a.off_budget && (a.type === "savings" || a.type === "investment")
  );
  for (const acc of linkedAccounts) {
    const key = acc.type; // "savings" or "investment"
    byAssetType[key] = (byAssetType[key] ?? 0) + acc.balance_inr;
  }
