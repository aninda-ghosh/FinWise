...
export async function fetchCurrentPrice(investment: InvestmentResponse): Promise<PriceResult> {
...
async function fetchMutualFundNAV(
  ...
  try {
    ...
    const funds: { schemeCode: number; schemeName: string }[] = await searchRes.json();
