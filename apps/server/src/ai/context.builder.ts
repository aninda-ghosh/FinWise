...
function labelAssetType(t: string) {
  const map: Record<string, string> = {
    mutual_fund: "Mutual Fund", stock: "Stock", etf: "ETF",
    fixed_deposit: "Fixed Deposit", bond: "Bond", real_estate: "Real Estate",
    cash: "Cash", structured: "Structured Product", other: "Other",
  };
  return map[t] ?? t;
}
...
function fromINR(inr: number, currency: string, rates: Record<string, number>): number {
...
export async function buildSystemContext(displayCurrency = "INR"): Promise<string> {
  const db = getDb();
  const today = new Date().toISOString().slice(0, 10);
  const currentMonth = today.slice(0, 7);

  const [netWorth, accounts_, envelopes, investments, policies, rates] = await Promise.all([
    getNetWorth(),
    listAccounts(),
  ...
  const lines: string[] = [
    ...
    Object.entries(rates).map(([k, v]) => `  1 ${k} = ₹${v}`).join("\n"),
    "",
    `## Bank & Savings Accounts`,
    ...(accounts_.length === 0 ? ["  No accounts added."] : accounts_.map(a => {
      const nativeLine = `${fmt(a.balance, a.currency)}${a.currency !== "INR" ? ` (${fmt(a.balance_inr, "INR")})` : ""}`;
      const displayLine = displayCurrency !== "INR" && displayCurrency !== a.currency
    ...
    ...(investments.length === 0 ? ["  No investments tracked."] : investments.map(i => {
      const purchaseDisplay = fmtDisplay(i.purchase_value * (i.currency === "INR" ? 1 : (rates[i.currency] ?? 1)));
      const currentDisplay = fmtDisplay(i.current_value_inr);
      const gainDisplay = fmtDisplay(i.gain_loss_inr);
      return `  ${i.name} — ${labelAssetType(i.asset_type)}, held in ${i.currency}` +
    ...
    ...(policies.length === 0 ? ["  No policies tracked."] : policies.map(p =>
