...
budgetRouter.patch("/accounts/:id", async (c) => {
  ...
  try {
    const account = await budgetService.updateAccount(c.req.param("id"), parsed.data);
...
budgetRouter.patch("/envelopes/:id", async (c) => {
  ...
  try {
    const envelope = await budgetService.updateEnvelope(c.req.param("id"), parsed.data);
...
budgetRouter.patch("/envelope-groups/:id", async (c) => {
  ...
  if (!body?.name?.trim()) return c.json({ error: "name required" }, 400);
  try {
    const group = await budgetService.updateEnvelopeGroup(c.req.param("id"), body.name.trim());
    return c.json(group);
  } catch (err) {
    return handleError(c, err);
  }
...
budgetRouter.post("/transactions", async (c) => {
  ...
  try {
    ...
    if (txn === null) return c.json({ skipped: true, reason: "duplicate" }, 409);
...
budgetRouter.patch("/transactions/:id", async (c) => {
  ...
  try {
    const txn = await budgetService.updateTransaction(c.req.param("id"), parsed.data);
    return c.json(txn);
  } catch (err) {
    return handleError(c, err);
  }
...
budgetRouter.post("/transactions/transfer", async (c) => {
  const body = await c.req.json().catch(() => null);
  if (!body?.from_account_id || !body?.to_account_id || !body?.amount || !body?.date) {
    return c.json({ error: "from_account_id, to_account_id, amount, date required" }, 400);
  }
  try {
...
budgetRouter.get("/reports/envelope-trends/:id", async (c) => {
  ...
  try {
    const trends = await budgetService.getEnvelopeTrends(c.req.param("id"), months);
