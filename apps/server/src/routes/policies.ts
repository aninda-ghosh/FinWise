...
policiesRouter.patch("/:id", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: "Validation failed", details: parsed.error }, 400);
  try {
    const policy = await policyService.updatePolicy(c.req.param("id"), parsed.data);
...
policiesRouter.post("/:id/payouts/generate", async (c) => {
  const body = await c.req.json().catch(() => null);
  if (!body || !body.start_date || !body.end_date || !body.amount || !body.frequency || !body.label) {
    return c.json({ error: "start_date, end_date, amount, frequency, label are required" }, 400);
  }
  try {
...
policiesRouter.post("/:id/payouts", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: "Validation failed", details: parsed.error }, 400);
  try {
    const payout = await policyService.addPayout(c.req.param("id"), parsed.data);
