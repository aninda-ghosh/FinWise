...
await sql`
  CREATE TABLE IF NOT EXISTS exchange_rates (
    ...
    rate_to_inr DOUBLE PRECISION NOT NULL,
    source TEXT,
    fetched_at TEXT
  )
`;
...
(async () => {
  try {
    ...
    if (count > 0) console.log(`[recurring] Applied ${count} due recurring transaction(s).`);
  } catch (err) {
    console.warn("[recurring] Failed to apply due recurring transactions:", (err as Error).message);
  }
})();
...
(async () => {
  try {
    ...
    console.warn("[startup] Could not refresh exchange rates:", (err as Error).message);
  }
})();
