...
export function useExchangeRates() {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<{ rates: { from_currency: string; rate_to_inr: number }[] }>("/api/exchange-rates").then(d => {
        const map: Record<string, number> = {};
        for (const r of d.rates) map[r.from_currency] = r.rate_to_inr;
        return map;
      }),
...
export function useEnvelopeGroups() {
  return useQuery({ queryKey: ["envelope-groups"], queryFn: budgetApi.getEnvelopeGroups });
}
...
export function useUpdateEnvelopeGroup() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) => budgetApi.updateEnvelopeGroup(id, name),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["envelope-groups"] }),
  });
}
...
export function useUpdateAccount() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => budgetApi.updateAccount(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["accounts"] });
      qc.invalidateQueries({ queryKey: ["net-worth"] });
      qc.invalidateQueries({ queryKey: ["dashboard"] });
...
export function useUpdateEnvelope() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => budgetApi.updateEnvelope(id, data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["envelopes"] }),
  });
}
...
export function useUpdateTransaction() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => budgetApi.updateTransaction(id, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["transactions"] });
      qc.invalidateQueries({ queryKey: ["envelopes"] });
      qc.invalidateQueries({ queryKey: ["monthly-summary"] });
...
export function useRecurring() {
  return useQuery({ queryKey: ["recurring"], queryFn: budgetApi.listRecurring });
}
export function useCreateRecurring() {
  const qc = useQueryClient();
  return useMutation({ mutationFn: budgetApi.createRecurring, onSuccess: () => qc.invalidateQueries({ queryKey: ["recurring"] }) });
}
export function useUpdateRecurring() {
  const qc = useQueryClient();
  return useMutation({ mutationFn: ({ id, data }: { id: string; data: any }) => budgetApi.updateRecurring(id, data), onSuccess: () => qc.invalidateQueries({ queryKey: ["recurring"] }) });
}
export function useDeleteRecurring() {
  const qc = useQueryClient();
  return useMutation({ mutationFn: budgetApi.deleteRecurring, onSuccess: () => qc.invalidateQueries({ queryKey: ["recurring"] }) });
}
export function useApplyDueRecurring() {
  const qc = useQueryClient();
  return useMutation({ mutationFn: budgetApi.applyDueRecurring, onSuccess: () => { qc.invalidateQueries({ queryKey: ["transactions"] }); qc.invalidateQueries({ queryKey: ["accounts"] }); qc.invalidateQueries({ queryKey: ["recurring"] }); } });
}
