...
export const budgetApi = {
  ...
  deleteAccount: (id: string) =>
    apiFetch<{ success: boolean }>(`${BASE}/accounts/${id}`, { method: "DELETE" }),

  getEnvelopes: (month: string) =>
    apiFetch<{ envelopes: EnvelopeListResponse }>(`${BASE}/envelopes?month=${month}`),
  ...
  deleteEnvelope: (id: string) =>
    apiFetch<{ success: boolean }>(`${BASE}/envelopes/${id}`, { method: "DELETE" }),

  reclaimEnvelope: (id: string) =>
    apiFetch<{ reclaimed_inr: number }>(`${BASE}/envelopes/${id}/reclaim`, { method: "POST" }),

  getEnvelopeGroups: () =>
    apiFetch<{ groups: { id: string; name: string; sort_order: number }[] }>(`${BASE}/envelope-groups`),

  createEnvelopeGroup: (name: string) =>
    apiFetch<{ id: string; name: string; sort_order: number }>(`${BASE}/envelope-groups`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  deleteEnvelopeGroup: (id: string) =>
    apiFetch<{ success: boolean }>(`${BASE}/envelope-groups/${id}`, { method: "DELETE" }),

  getTransactions: (filters: Partial<TransactionFilters>) => {
    const params = new URLSearchParams();
    Object.entries(filters).forEach(([k, v]) => v != null && params.set(k, String(v)));
    return apiFetch<PaginatedTransactionsResponse>(`${BASE}/transactions?${params}`);
  },
  ...
  createRecurring: (data: any) => apiFetch<any>(`${BASE}/recurring`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }),
  updateRecurring: (id: string, data: any) => apiFetch<any>(`${BASE}/recurring/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }),
  deleteRecurring: (id: string) => apiFetch<{ success: boolean }>(`${BASE}/recurring/${id}`, { method: "DELETE" }),
  applyDueRecurring: () => apiFetch<{ applied: number }>(`${BASE}/recurring/apply-due`, { method: "POST" }),
};
