...
const mdComponents = {
  ...
  code: ({ children }: any) => <code className="font-mono text-xs bg-black/10 dark:bg-white/10 rounded px-1 py-0.5">{children}</code>,
  pre: ({ children }: any) => <pre className="font-mono text-xs bg-black/10 dark:bg-white/10 rounded p-2 my-1 overflow-x-auto whitespace-pre-wrap">{children}</pre>,
...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${isUser ? "bg-indigo-500" : "bg-muted border"}`}>
        {isUser
...
export default function ChatPage() {
  ...
  const [aiStatus, setAiStatus] = useState<"checking" | "available" | "no_model" | "unavailable">("checking");
  ...
  const checkStatus = () => {
    chatApi.getStatus()
      .then(d => {
        ...
        if (!d.connected) { setAiStatus("unavailable"); return; }
        // Check if the *user-selected* model is available
        const modelOk = models.some(m => m === aiModel || m.startsWith(aiModel));
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {availableModels.length > 1 ? (
              <div className="relative">
                <select
                  value={aiModel}
                  onChange={e => { setAiModel(e.target.value); setTimeout(checkStatus, 0); }}
              ...
              <p className="text-xs text-muted-foreground leading-tight">{aiModel} · local</p>
            )}
          </div>
          {aiStatus === "checking" && (
          ...
          {aiStatus === "no_model" && (
            <span className="inline-flex items-center gap-1 text-xs text-red-500 ml-1">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500 inline-block" />Model missing
            </span>
          )}
        </div>
        <Button
          ...
          onClick={() => clearChat(undefined, { onSuccess: () => toast.success("Chat cleared") })}
        >
      ...
      {aiStatus === "unavailable" && (
        <div className="mx-4 mt-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 px-4 py-3 flex items-center justify-between gap-4 flex-shrink-0">
          ...
          <Button
            ...
            onClick={startOllama}
          >
            {startingOllama ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Starting…</> : <><Play className="w-3.5 h-3.5" />Start Ollama</>}
          </Button>
        </div>
      )}
      {aiStatus === "no_model" && (
            ...
            <div className="flex-1 min-w-0">
              ...
              {availableModels.length > 0 && (
                <p className="text-xs text-red-600 dark:text-red-400 mt-1.5">
                  Installed models: <span className="font-mono">{availableModels.join(", ")}</span>
                </p>
              )}
            </div>
          </div>
        ...
        {messages.length === 0 && !streaming ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 pt-20 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">
              Ask me anything about your finances — net worth, budget, investments, policies, or savings goals.
            </p>
      ...
      <div className="border-t px-4 py-3 flex-shrink-0">
        <div className="max-w-3xl mx-auto flex gap-2 items-end">
          <Textarea
            ...
            placeholder={isUnavailable ? "AI bot not available…" : "Ask about your finances… (Enter to send, Shift+Enter for newline)"}
