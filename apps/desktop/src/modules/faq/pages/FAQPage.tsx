    ...
    items: [
      {
        ...
        a: (
          <span>
            Envelope budgeting means you allocate every pound/dollar of income to a named
            "envelope" (category) before you spend it. When you add income, it becomes
            available to assign. You then budget amounts into envelopes like Groceries,
            Rent, or Savings. Transactions are tagged to envelopes, reducing their
            balance. This gives you a real-time view of what you have left in each
            category — not just your bank balance.
          </span>
        ),
      },
      {
        ...
        a: (
          <span>
            Use <strong>Budget → Add Transaction → Transfer</strong>. Select a From and
            To account and enter the amount. If the accounts are in different currencies,
            enter both the sent and received amounts. This creates two linked transactions
        ...
        a: (
          <span>
            <ol className="list-decimal ml-4 space-y-1">
              <li>
                Add an <strong>Investment-type account</strong> (e.g. "Brokerage") via
                Add Account.
              </li>
              <li>
                <strong>Transfer</strong> money from Checkings → Brokerage account. This
                moves the cash correctly.
              </li>
              <li>
                Add the holding in <strong>Investments</strong> with the purchase value.
              </li>
            </ol>
            <p className="mt-2">
              Investment-type accounts are automatically excluded from the "Cash &amp;
              Accounts" net worth total, since that value is already captured in the
              Investments module. No double-counting.
            </p>
          </span>
        ),
      },
      ...
      },
      {
        ...
        a: (
          <span>
            <p>CSV import is available via the API at <code className="bg-muted px-1 rounded text-xs">POST /api/budget/import?account_id=…</code>.</p>
            <p className="mt-2">The CSV must have these exact column headers:</p>
            <code className="block bg-muted px-2 py-1 rounded text-xs mt-1">date, payee, amount, type, notes</code>
            <ul className="list-disc ml-4 mt-2 space-y-1">
              <li><strong>date</strong> — ISO format (YYYY-MM-DD)</li>
              <li><strong>type</strong> — one of <code className="bg-muted px-1 rounded text-xs">income</code>, <code className="bg-muted px-1 rounded text-xs">expense</code>, <code className="bg-muted px-1 rounded text-xs">transfer</code></li>
              <li><strong>notes</strong> — optional</li>
            </ul>
            <p className="mt-2">Duplicate rows are automatically skipped using a SHA-256 hash of each row, so re-importing the same file is safe. Note: most banks export CSVs with different column names — you will need to rename the columns to match the format above before importing.</p>
          </span>
        ),
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: (
          <span>
            <code className="bg-muted px-1 rounded text-xs">Gain/Loss = current_value − purchase_value</code>
            {" "}(both converted to INR using stored exchange rates for cross-currency comparison).
            The percentage is <code className="bg-muted px-1 rounded text-xs">(gain_loss_inr / purchase_value_inr) × 100</code>.
          </span>
        ),
      },
      {
        q: "What does the Refresh Price button do?",
        a: "It records the current stored value as a price history entry and updates the 'last updated' timestamp. There is no live market price fetching — prices are always what you manually enter. Use it to snapshot a value at a point in time.",
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: (
          <span>
            Check the Ollama status at the top of the Chat page. If it shows Offline,
        ...
        a: (
          <span>
            ...
            <ul className="list-disc ml-4 mt-1 space-y-1">
              <li><strong>Clear Transactions</strong> — deletes all transactions and resets envelope budgets to zero, but keeps your accounts, envelope groups, and envelope names intact.</li>
              <li><strong>Reset All Data</strong> — permanently deletes everything: accounts, transactions, envelopes, investments, policies, debt, and AI chat history. Requires typing a confirmation phrase. This action is irreversible.</li>
            </ul>
          </span>
        ),
      },
